...
import { IconSparkles, IconLoader2, IconBolt, IconPhoto, IconVideo } from '@tabler/icons-react'
  ...
  return (
    <div className="min-h-screen flex bg-white">
      ...
      <div className="hidden lg:flex lg:w-[42%] flex-col justify-between p-12 bg-[#09090B]">
        ...
        <div>
          <p className="text-[10px] font-bold tracking-[0.14em] uppercase text-[#52525B] mb-5">
          ...
          <div className="space-y-3">
            {FEATURES.map(({ icon: Icon, label }) => (
              <div key={label} className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-[7px] bg-[#18181B] flex items-center justify-center flex-shrink-0">
                  <Icon size={14} stroke={1.8} className="text-[#A1A1AA]" />
                </div>
          ...
          <div className="flex gap-8">
            {[{ v: '500+', l: 'Marcas activas' }, { v: '10K+', l: 'Assets generados' }, { v: '$0', l: 'Para empezar' }].map(({ v, l }) => (
              <div key={l}>
                ...
                <div className="text-[10px] text-[#52525B] font-medium uppercase tracking-wide mt-0.5">{l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 sm:px-10 bg-[#FAFAFA]">
        <div className="w-full max-w-[380px]">
          ...
          <h1 className="text-xl font-semibold text-[#09090B] tracking-tight mb-1">Iniciar sesión</h1>
          <p className="text-sm text-[#71717A] mb-7">Bienvenido de vuelta a tu cuenta</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-2.5">
              {loading ? <><IconLoader2 size={15} className="animate-spin" /> Iniciando sesión…</> : 'Iniciar sesión'}
            </button>
          </form>
          ...
          <p className="text-center text-xs text-[#71717A] mt-6">
            ¿No tienes cuenta?{' '}
            <Link href="/register" className="text-[#09090B] font-semibold hover:underline">Regístrate gratis</Link>
          </p>
        </div>
      </div>
    </div>
